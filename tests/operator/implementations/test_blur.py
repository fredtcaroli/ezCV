...
@parametrize_img
def test_gaussian_blur_run_return_img(blur_op, img):
    r = blur_op.run(img)
    assert is_image(r)


@parametrize_img
def test_gaussian_blur_run_return_same_shape(blur_op, img):
    r = blur_op.run(img)
    assert r.shape == img.shape


@parametrize_img
def test_gaussian_blur_run_leave_input_untouched(blur_op, img):
    before_run = img.copy()
    _ = blur_op.run(img)
...
@parametrize_img
def test_gaussian_blur_run_normalize_kernel_size(blur_op, img):
    blur_op.kernel_size = 3
    result_odd = blur_op.run(img)
    blur_op.kernel_size = 4
    result_even = blur_op.run(img)
